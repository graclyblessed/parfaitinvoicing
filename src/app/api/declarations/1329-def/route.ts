...
export async function GET(request: NextRequest) {
  try {
    ...
    const inputs: CVAEInput = {
      ...
      reductionSupplementaire: existing?.ligne10_reduction_supplementaire ?? 0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fullInputs: CVAEInput = {
      ...
      isServices: inputs?.isServices ?? true,
      exonerations: inputs?.exonerations ?? 0,
      reductionSupplementaire: inputs?.reductionSupplementaire ?? 0,
